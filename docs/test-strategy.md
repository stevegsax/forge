...
## 1) General approach
- Favour the testing pyramid: most coverage from fast, isolated unit tests; a thinner layer of integration tests for cross-module seams; a small set of end-to-end or workflow checks for critical user paths.citeturn1search7
- Keep the suite green and quick; slow or flaky tests erode trust and are ignored.citeturn0search0
...
- Follows the F.I.R.S.T./FIRST-U heuristic: Fast, Isolated, Repeatable, Self-validating, Timely/Understandable.citeturn0search3
- Has one clear reason to fail; asserts a single behaviour or invariant.
- Uses minimal, explicit fixtures; avoids hidden global state.citeturn1search0
...
- Flaky (sometimes passes, sometimes fails) — usually relies on uncontrolled state, time, or external services.citeturn0search0
...
- Use `conftest.py` fixtures for shared setup (e.g., temp git repo, ruff config) and keep them focused.citeturn1search0
- Mock or fake external systems (Temporal server, network) to keep tests fast and isolated; prefer fakes over deep mocking when behaviour matters.citeturn2search7
...
## 7) What to avoid
- Hitting live network, real clocks, or random seeds without control — primary sources of flakiness.citeturn0search0
- Long-running end-to-end UI-style tests as a substitute for missing unit coverage; keep top-of-pyramid tests minimal.citeturn1search7
...
- **Pytest fixtures** for controlled, reusable setup; scope them appropriately (function by default; session for expensive resources).citeturn1search0
- **Ruff** for linting to keep tests tidy and consistent.
- **uv** for locked, reproducible environments; prefer `uv run pytest` to ensure deps match `uv.lock`.citeturn1search3
