...
## 1) General approach
- Favour the testing pyramid: most coverage from fast, isolated unit tests; a thinner layer of integration tests for cross-module seams; a small set of end-to-end or workflow checks for critical user paths.citeturn1search5
...
- Follows the F.I.R.S.T./FIRST-U heuristic: Fast, Isolated, Repeatable, Self-validating, Timely/Understandable.citeturn0search7
- Has one clear reason to fail; asserts a single behaviour or invariant.
- Uses minimal, explicit fixtures; avoids hidden global state.citeturn0search1
...
- Flaky (sometimes passes, sometimes fails) — usually relies on uncontrolled state, time, or external services.citeturn0search2
...
- Use `conftest.py` fixtures for shared setup (e.g., temp git repo, ruff config) and keep them focused.citeturn0search1
- Mock or fake external systems (Temporal server, network) to keep tests fast and isolated.citeturn0search2
...
## 7) What to avoid
- Hitting live network, real clocks, or random seeds without control — primary sources of flakiness.citeturn0search8
- Long-running end-to-end UI-style tests as a substitute for missing unit coverage; keep top-of-pyramid tests minimal.citeturn1search5
...
- **Pytest fixtures** for controlled, reusable setup; scope them appropriately (function by default; session for expensive resources).citeturn0search1
