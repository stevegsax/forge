...
- **This repo is the monorepo** (D98, T2.1 complete 2026-07-16): one uv workspace, one lock, one venv — `apps/pbook`, `apps/ocr`, `libs/sax-llm`, and `libs/sax-platform` (added by Phase 3, T3.1). A bare clone is self-contained; Python is pinned to **3.14** (standard GIL). The five predecessor repos are archived; `libs/sax-platform` is new to Phase 3, not a predecessor. `libs/forge-contracts` was absorbed into `libs/sax-platform` and retired as a package identity at T3.4 — the workspace is **five packages today** (forge, pbook, ocr, sax-llm, sax-platform).
...
Module paths are under `src/forge/` unless a workspace member (`sax_llm.*`, `sax_platform.*`) is named explicitly.
...
**Tooling & ops debt** (surfaced 2026-07-16 during the monorepo/deployment work; Phase 2 closed the CI/gates/findings rows — GitHub Actions CI, import-linter DAG contracts, all six workspace packages at the time gated at 85% coverage, mypy strict across the workspace (T2.3a–d), the four standing findings fixed; T3.4 subsequently retired `forge-contracts`, leaving five packages):
