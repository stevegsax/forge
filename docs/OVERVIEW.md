...
- **OCR consumer (cross-queue)** — OCR lives in the `apps/ocr` workspace member and owns its full Mistral batch lifecycle (submit, timer-loop poll, fetch/store) via `sax_platform.ocr.MistralOcr`; it shares none of forge's transport (T4.2 deleted the batch SPI). The only cross-queue link is the shared batch ledger: ocr writes `batch_jobs` rows via `persist_block` activity calls on `forge-task-queue` and reads that ledger read-only, both from `sax_platform.contracts` (the former `forge-contracts` package, absorbed at T3.4); neither package imports the other.
...
| `requirements/human_in_the_loop.feature` | **Not built** — no structured pause/resume/approval primitive | No `HumanInputRequest`/`emit_user_prompt`; zero `@workflow.signal` platform-wide (batch and OCR both poll on timer loops); human gating is out-of-band git review + manual playbook approval |
...
| Human-in-the-loop | Only out-of-band merge gating + manual playbook approval; no structured intervention and no signal primitive (batch/OCR both poll on timer loops) | `manual_playbook_workflow.py` |
