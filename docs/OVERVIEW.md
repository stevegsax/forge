...
- **Batch** — per-workflow timer-loop transport (D88, T4.1): each workflow submits, then polls `batch_status` on a sleep loop until the batch ends and fetches its own result (no shared poller, no signal). Forge's own LLM calls use the Anthropic Batch API; the opaque-blob SPI additionally routes OCR submissions to the Mistral batch API via `sax_platform.ocr.MistralOcr`: `workflow_blocks.py`, `activities/batch_*`.
...
- **Batch SPI (OCR-agnostic)** — opaque-blob batch submit (`submit_batch_blob`); the consumer workflow polls `batch_status` and fetches its own verbatim provider result cross-queue via `fetch_batch_result`: `activities/batch_submit.py`, `activities/batch_fetch.py`. OCR itself lives in the `apps/ocr` workspace member, consuming the platform via `sax_platform.contracts` (the former `forge-contracts` package, absorbed at T3.4); neither package imports the other.
