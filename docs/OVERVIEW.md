...
- **Migration status:** Phases 1–2 done (root CI, 85% gates across the workspace, import-linter DAG contracts, mypy strict everywhere); Phase 3 in progress — T3.1–T3.3 landed the `sax-platform` package (its LLM client both lanes, the tiers registry, and the Mistral OCR capability); T3.4 landed the platform plumbing modules (`sax_platform.temporal`/`db`/`embeddings`/`config`/`logging`) and the `sax_platform.contracts` sandbox-light layer, retiring `forge-contracts` and migrating forge/ocr/pbook onto the platform; T3.5 retired forced tool use platform-wide (forge and pbook now complete structured outputs through `sax_platform.llm`, frozen `OUTPUT_TYPES` replace the string registries, `libs/sax-llm` deleted); T3.6 remains to close Phase 3; Phases 4–8 ahead. Live queue: [../development-plans/TASKS.md](../development-plans/TASKS.md).
