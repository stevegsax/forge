"""S3 blob storage for OCR image and file bytes.

Imperative shell: all S3 I/O for OCR blobs is encapsulated here so the store
functions keep their byte-in / byte-out signatures. The bucket is configured by
``FORGE_OCR_S3_BUCKET`` (required for any OCR work); an optional
...
S3 is the only OCR blob store: an unset bucket or an S3 error raises, which fails
the OCR task (the worker and non-OCR work are unaffected). There is no inline-in-DB
fallback and no runtime failover.
"""
...
def get_bucket() -> str:
    """Return the configured OCR blob bucket, or raise if unset."""
    bucket = os.environ.get("FORGE_OCR_S3_BUCKET")
    if not bucket:
        raise S3ConfigError(
            "FORGE_OCR_S3_BUCKET is not set; OCR blob storage requires an S3 bucket."
        )
    return bucket
...
def _client():
    # boto3 is heavy; import lazily so non-OCR code paths don't pay for it. A
