...
- [~] C7. OCR **submit** DONE: `submit_ocr_batch` does no store writes (no `record_batch_submission`/`record_batch_failure`/`delete_file_content`) — it just returns `OcrBatchRef` or raises; `OcrSubmitWorkflow` persists `PersistBatchSubmission` then deletes the blob via a new `delete_file_content_blob` activity, and persists `PersistBatchFailure` on API error (fixes the double-submit-on-DB-error bug). Batch-poll image ids already deterministic (C6). **Remaining:** OCR sync (move the `ocr_results` write from `call_ocr_sync` into `OcrSyncWorkflow` via `PersistOcrResult` — a return-type refactor) and bump store/reassembly call-site retries to `_PERSIST_RETRY`.
