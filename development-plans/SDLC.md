...
| Operator | External repos (`sax-datastores`, `sax-temporal`), administrator-run | Infrastructure strategy, configuration, and operation: the shared Postgres and Temporal stacks, credentials, backups, schema-change intake/review/apply, namespace intake |
...
The operators are counterparties, not agents of this repo
(consumer/operator model, owner-ratified 2026-08-02). Consumers
request via committed artifacts and issues on the operator's repo; the
reverse channel is operator tickets on consumer repos; nobody writes
across the boundary in either direction, and agents on either side
never apply schema changes (§22 of the canonical process,
`sax-datastores/docs/schema-changes.md`). The permission sandbox that
denies writes into the operator repos enforces this model — it is a
...
9. **Deploy — decoupled.** Owner-triggered, never implied by task
   ...
   A deploy whose code carries a new schema revision waits for the
   operator's apply: verify-only startup (2026-08-03) refuses a
   schema behind the code, so the wrong order fails closed at worker
...
## Operator requests (request-and-wait)

Forge consumes infrastructure it does not operate (D104): the shared
Postgres stacks are sax-datastores', the Temporal stacks are
sax-temporal's. When a task needs anything on the far side of that
boundary — DDL against a shared database, an extension, a role, a
namespace — it enters a **request-and-wait** state: forge commits a
request artifact, opens an issue on the operator's repo, and blocks on
the operator's intake → review → apply. The task file stays IN
PROGRESS with the issue link and the wait recorded in Development
Notes; dev/test remain self-service in the meantime (`forge migrate` /
`ocr migrate` / `pbook migrate`). Forge-side mechanics:
[PROCESS.md — Schema Changes](PROCESS.md#schema-changes-operator-requests).
The canonical process — tiers, the auto-reject list, the apply
runbook — is the operator's `sax-datastores/docs/schema-changes.md`;
link it, never duplicate it (theory rule 1).
...
| `datastore-changes/` (root: product forge, both chains) and `apps/pbook/datastore-changes/` (product pbook) | Committed schema-change requests: per-phase offline SQL + `request.md`; the pinned commit *is* the request | Generated by `make db-change`; linted by `make lint-sql` inside `make gates`; immutable once submitted — a new commit voids approval, so any change is a new request |
| `.squawk.toml` (repo root) | Byte-copy of the operator's canonical Squawk config (`sax-datastores/templates/squawk.toml`) | Re-copied when the operator's template changes; never edited locally — a consumer-local edit voids every lint claim |
...
The operator's own records — `registrations/<slug>.yaml`, the
per-request `schema-changes/<product>/<id>.yaml` apply records, and the
canonical process doc — live in the operator's repo and are theirs to
keep true; forge cross-references and never edits them.
...
### Consumer/operator boundary (2026-08-02 agreement)

- No product DDL, ever: workers verify the schema at startup and
  refuse when behind; schema changes are Alembic revisions requested
  through the change-request process and applied by the operator.
  Agents on either side never apply (§22).
- Expand/contract is binding: every request leaves currently-deployed
  code working; a breaking change is three requests
  (expand → migrate → contract).
- The commit is the request: nothing under a submitted
  `datastore-changes/` request directory is ever modified — a
  revision is a new request.
- Never write into the operator repos (`~/repos-sax/sax-datastores`,
  `~/repos-sax/sax-temporal`) and never read
  `~/.config/sax-datastores` (secrets; credential delivery is the
  owner pasting URLs into profiles — presence checks only, never
  print values).
- Deploy order: code carrying a new schema revision deploys only
  after the operator applies the change.
