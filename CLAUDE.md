...
**Landed** (as of 2026-07-16): **Phase 1** (T1.0–T1.8); **T2.1** — the monorepo is complete (all four packages absorbed as workspace members, D98; Python pinned to 3.14); **T0.7** — EC2 retired for the local-first deployment (D99); **T2.2** — root gates: GitHub Actions CI, import-linter DAG contracts, every package gated at 85% coverage (`make gates` runs exactly what CI runs); **T2.3a–b** — forge-contracts and sax-llm are mypy-strict (zero ignores), member mypy wired into `make typecheck` and CI. **Next up: T2.3c–d** (mypy strict for ocr, pbook), which close Phase 2. Phase 0's other tasks (T0.1–T0.6, T0.8) are independent of every phase and can land anytime. Phases 3–8 are ahead.
