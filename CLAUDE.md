...
**Landed** (as of 2026-07-16): **Phase 1** (T1.0–T1.8); **T2.1** — the monorepo is complete (all four packages absorbed as workspace members, D98; Python pinned to 3.14); **T0.7** — EC2 retired for the local-first deployment (D99); **T2.2** — root gates: GitHub Actions CI, import-linter DAG contracts, every package gated at 85% coverage (`make gates` runs exactly what CI runs); **T2.3a–d** — every workspace package is mypy-strict, member mypy wired into `make typecheck` and CI (zero ignores except 16 recorded temporalio stub-boundary ignores in pbook's `workflows/cli_ops.py`). **Phase 2 is complete.** Phase 3 is underway: **T3.1** — `libs/sax-platform` born with `sax_platform.llm`, the structured-outputs client on both lanes (typed refusal/truncation outcomes, `max_retries=0`, required `max_tokens`, opt-in caching); **T3.2** — `sax_platform.llm.tiers` is the single tier registry (REASONING → opus-4-8, GENERATION/SUMMARIZATION → sonnet-5, CLASSIFICATION → haiku-4-5; pins live-verified, D94 note), thinking is adaptive + explicit effort platform-wide (`budget_tokens` deleted; on the new pins omission runs adaptive by default, so "off" is an explicit disabled shape), forge resolves models only through the registry (CLI: `--effort`, not `--thinking-budget`), and forge now depends on sax-platform. **T3.3** — `sax_platform.ocr` owns the Mistral OCR capability (`MistralOcr`, injected client); forge's SPI mistral branch uses it; sax-llm's Mistral support (incl. the zero-users chat lane) is deleted and `mistralai` moved to sax-platform; ocr installs the DI seam at worker startup. *(Post-merge operator action: run the env-gated ocr e2e once the launchd workers restart on main — command in the T3.3 task file.)* **Next up: T3.4** (platform plumbing modules). Phase 0's other tasks (T0.1–T0.6, T0.8) are independent of every phase and can land anytime. Phases 3–8 are ahead.
...
- **sax-llm** (`libs/sax-llm`, workspace member) — shared LLM provider abstraction, output type registry, and batch response parsing. (Superseded across Phase 3 by sax-platform; retired when consumers finish migrating.)
- **sax-platform** (`libs/sax-platform`, workspace member; **not yet a dependency of anything**) — the D89 platform library, born in T3.1 with `sax_platform.llm`: structured-outputs LLM client on both lanes (sync `messages.create`+`output_config.format`, batch request builder + submit/status/fetch helpers), typed refusal/truncation/mismatch outcomes, opt-in prompt caching. Consumers migrate in T3.2/T3.5/T3.6; T3.4 adds the plumbing modules.
- **ocr** (`apps/ocr`, workspace member; **not** a forge dependency) — document OCR via the Mistral batch API, consuming the platform through the batch SPI. Own Temporal worker on `ocr-task-queue`; cross-queue SPI calls to `forge-task-queue`; own Alembic chain (`alembic_version_ocr`, four `ocr_*` tables) in the shared `FORGE_DB_URL` database; reads `batch_jobs` read-only via the forge-contracts mirror. Imports `forge_contracts` and `sax_platform` (the `MistralOcr` DI seam in `ocr/deps.py`, installed at worker startup since T3.3 for Phase 4's self-polling), never `forge`.
