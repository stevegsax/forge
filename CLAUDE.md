...
**Landed** (as of 2026-07-17): **Phases 1 and 2 are complete.** Phase 1 (T1.0–T1.8) stopped the bleeding; Phase 2 delivered the monorepo (T2.1, D98; Python pinned to 3.14), the root gates + GitHub Actions CI (T2.2 — `make gates` mirrors CI; import-linter DAG contracts; every package coverage-gated at 85%), and mypy-strict across all six packages (T2.3a–d). **T0.7** retired EC2 for the local-first deployment (D99). **Phase 3 is underway:** T3.1–T3.3 landed the `sax_platform` library — the both-lane structured-outputs LLM client, the single model-tier registry, and `MistralOcr` — and made both forge and ocr depend on it. **Next up: T3.4** (platform plumbing modules); T3.5/T3.6 then close Phase 3. Phase 0's other tasks (T0.1–T0.6, T0.8) are independent of every phase and can land anytime; Phases 4–8 are ahead. Per-task detail is in [development-plans/TASKS.md](development-plans/TASKS.md) and [development-plans/CHANGELOG.md](development-plans/CHANGELOG.md).

**Still current until later phases — do not "fix" these as if done:** forge's runtime LLM completion path still runs through sax-llm's Anthropic provider with forced tool use until T3.5/T3.6 (sax_platform already supplies the tier registry and the Mistral-OCR capability, but not yet the LLM client); the signal-based batch transport and its shared poller live until Phase 4 (T4.1); pbook keeps its own duplicated tier map until T6.4.
...
- **sax-llm** (`libs/sax-llm`, workspace member) — shared Anthropic LLM provider abstraction, output-type registry, and batch response parsing. Its Mistral support (including the zero-users chat lane) was deleted in T3.3 and `mistralai` moved to sax-platform, so it is Anthropic-only now. Forge's runtime LLM completion path still runs through it until T3.5/T3.6. Superseded across Phase 3 by sax-platform; retired when consumers finish migrating.
- **sax-platform** (`libs/sax-platform`, workspace member; **a dependency of both forge and ocr**) — the D89 platform library. `sax_platform.llm`: structured-outputs LLM client on both lanes (sync `messages.create` + `output_config.format`, batch request builder + submit/status/fetch helpers), typed refusal/truncation/mismatch outcomes, `max_retries=0`, required `max_tokens`, opt-in prompt caching. `sax_platform.llm.tiers`: the single model-tier registry (REASONING → `claude-opus-4-8`, GENERATION/SUMMARIZATION → `claude-sonnet-5`, CLASSIFICATION → `claude-haiku-4-5`); `budget_tokens` is gone platform-wide — thinking is adaptive or explicitly disabled, and the forge CLI flag is `--effort`, not `--thinking-budget`. `sax_platform.ocr`: `MistralOcr` (owns the `mistralai` dependency). T3.4 adds the plumbing modules; T3.5/T3.6 finish the consumer migration.
- **ocr** (`apps/ocr`, workspace member; **not** a forge dependency) — document OCR via the Mistral batch API, consuming the platform through the batch SPI. Own Temporal worker on `ocr-task-queue`; cross-queue SPI calls to `forge-task-queue`; own Alembic chain (`alembic_version_ocr`, four `ocr_*` tables) in the shared `FORGE_DB_URL` database; reads `batch_jobs` read-only via the forge-contracts mirror. Imports `forge_contracts` and `sax_platform` (its `MistralOcr` DI seam lives in `ocr/deps.py` and is installed at worker startup, T3.3), never `forge`.
...
Workspace command discipline: the commands above, run from the root, cover forge only (`testpaths = ["tests"]` keeps the members out of the default run). Setup is `uv sync --all-packages` (a bare exact `uv sync` prunes ocr's packages, since forge doesn't depend on it; `uv run`'s inexact sync self-heals). Each member's suite runs from its own directory so its own config applies: `cd apps/pbook && uv run pytest` (85% gate; its conftest needs a running podman machine or `PBOOK_TEST_DATABASE_URL`), `cd apps/ocr && uv run pytest` (85% gate), `cd libs/sax-llm && uv run pytest` (85% gate), `cd libs/forge-contracts && uv run pytest` (85% gate), `cd libs/sax-platform && uv run pytest` (85% gate). `make gates` runs the workspace gates — lint, mypy (all six packages), lint-imports, and the six test suites — mirroring `.github/workflows/ci.yml`, which additionally runs the `postgres` migration suite (`pytest -m postgres`) as its own job. Never run a member's tests from the root (e.g. `pytest apps/pbook/tests`): forge's addopts would apply and the member's conftest would run under the wrong config.
